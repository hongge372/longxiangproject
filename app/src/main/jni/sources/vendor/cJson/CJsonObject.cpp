...
CJsonObject::CJsonObject(const std::string& strJson, bool isJsonStr)
    : m_pJsonData(NULL), m_pExternJsonDataRef(NULL)
{
    if (isJsonStr) Parse(strJson);
    else {
        std::ifstream ifile(strJson);
...
CJsonObject::CJsonObject(const char *strJson)
: m_pJsonData(NULL), m_pExternJsonDataRef(NULL)
{
    Parse(std::string(strJson));
}
